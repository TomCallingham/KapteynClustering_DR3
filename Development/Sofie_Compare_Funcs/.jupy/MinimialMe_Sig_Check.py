...
# for i in [3681]: #list(range(N_potential_clusters)):
for i in list(range(N_potential_clusters)):
    # art_region_counts[i], art_region_counts_std[i] = expected_density_parallel(i)
    # region_counts[i] = density_within_ellipse(i)
    members = tree_members[i+ Nclusters]
    region_counts[i], art_region_counts[i], art_region_counts_std[i] = sigf.cut_expected_density_members(members, N_sigma_ellipse_axis, X, art_X, N_art, min_members)
    # region_counts[i], art_region_counts[i], art_region_counts_std[i] = sigf.expected_density_members(members, N_sigma_ellipse_axis, X, art_X, N_art, min_members)
    ...
    if diff>1e-3:
        print("different")
        print(i)
        print(region_counts[i], s_region_counts[i])
        print(art_region_counts[i], art_region_counts_std[i])
        print(s_art_region_counts[i], s_art_region_counts_std[i])
        break


# %%
diff = np.abs(s_region_counts - region_counts) +  np.abs(s_art_region_counts - art_region_counts)\
    +  np.abs(s_art_region_counts_std - art_region_counts_std)

# %%
n_diff = (diff>1e-3).sum()
print(n_diff)

# %%
significance = (region_counts - art_region_counts)/np.sqrt( region_counts + (art_region_counts_std**2))
my_slow_data = {"region_count": region_counts,
                "art_region_count": art_region_counts,
                "art_region_count_std": art_region_counts_std,
                "significance": significance}
file_base = data_params["result_path"] + data_params["data_folder"]
save_name = data_params["sig"] + "_slow_minimal"
dicf.h5py_save(file_base + save_name, my_slow_data, verbose=True, overwrite=True)

# %%
diff = np.abs(significance- stats["significance"].values)
