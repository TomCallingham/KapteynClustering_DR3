...
def iso_fit_prob(data_arrays, fitting_params, plot=True, save_path=None):
# Fitting and determination of age and metallicity:
    '''
    Fits Isochrones to groups of stars.
    Inputs need:
    data_arrays containing: colour, colour_err, magnitude, extinction
    fitting_params: extinction_cut, percentile_solution, iso_models=PARSEC/BaSTI, alpha=SS (Solar Sample) or  AE (Alpha Enhanced), ranges
    ranges containing: colour, magnitude, iso_age, iso_met
    '''
    print('###############################')
    print(f'## Fittiing Isochrones ...  ##')
    print('###############################')
    ...
    if plot:
        ...
        iso_fit_plot(data_arrays, ranges, fitting_params, results, plotting_data, save_path)
...
### Plotting Script
# From Tomas
def iso_fit_plot(data_arrays, ranges, fitting_params, results, plotting_data, save_path=None):
    ...
    # ax2.plot([np.nan, np.nan], [np.nan, np.nan], 'bo', label='Cluster #' + str(total_name) + ', ' + str(len(colour_array)) + ' stars')
