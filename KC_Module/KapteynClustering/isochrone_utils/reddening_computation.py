...
import numpy as np
try:
    from . import lallement18_distant as l18
except Exception as e:
    print(e)
    import KapteynClustering.isochrone_utils.lallement18_distant as l18


def reddening_computation(df, dust_map="l18", ext_rec ="bab18"):
    "df a vaex dataframe"
    ...
    if dust_map == 'l18':
        ...
        delta_EBV, EBV  = l18_distant_reddening( df.evaluate('l'), df.evaluate('b'), df.evaluate('distance')*1000.0)
    elif dust_map == 'p19':
        # We use the dust map from pepita19. It would use dust_maps_3d/pepita19.py and the file pepita19.fits.gz
        delta_EBV, EBV  = p19_reddening( df.evaluate('l'), df.evaluate('b'), df.evaluate('distance')*1000.0)
    elif dust_map == 'bayestar':
        EBV, delta_EBV = bayestar_reddening(df.evaluate('l'), df.evaluate('b'), df.evaluate('distance'))
    else:
        raise AttributeError(f"dust map {dust_map} is not known")

    # From those E(B-V) compute the AG and E_BP_RP values
    if ext_rec == 'cas18':
        AG, delta_AG,  E_BP_RP, delta_BP_RP = cas18_extinction(df.evaluate("bp_rp"), EBV, delta_EBV)
    elif ext_rec == 'bab18':
        ### Babusiaux et al. 2018 --> http://aselfabs.harvard.edu/abs/2018A%26A...616A..10G (eq 1, Table 1)
        AG, delta_AG,  E_BP_RP, delta_BP_RP = bab18_extinction(df.evaluate("bp_rp"), EBV, delta_EBV)

    elif ext_rec == 'else':
        AG, delta_AG,  E_BP_RP, delta_BP_RP = else_extinction(EBV, delta_EBV)
    else:
        raise AttributeError(f"ext_rec {ext_rec} is not known")

    COL_bp_rp = df.evaluate('bp_rp') - E_BP_RP
    MG = df.evaluate('phot_g_mean_mag')-AG+5.-5.*np.log10(1000.0/df.evaluate('new_parallax'))

    return E_BP_RP, delta_BP_RP, AG, delta_AG, COL_bp_rp, MG
#return E_BP_RP, delta_E_BP_RP, AG, delta_AG, COL_bp_rp, MG

def l18_distant_reddening(l,b,distance):
    ebv = l18.L18Map()
    EBV = ebv(l, b, distance)
    delta_EBV = EBV*0.0
    return EBV, delta_EBV

def p19_reddening(l, b, distance):
    try:
        EBV = p19.ebv(l, b, distance)
    except Exception:
        raise AttributeError("p19 not loaded")
    delta_EBV = EBV*0.0
    return delta_EBV, EBV

def bayestar_reddening(l_deg, b_deg, dis_kpc):
    try:
        bayestar = BayestarQuery(max_samples=1, version='bayestar2019')
    except Exception:
        raise AttributeError("bayestar not loaded")
    from astropy.coordinates import SkyCoord
    from astropy import units as u
    coords = SkyCoord(l_deg*u.deg, b_deg*u.deg, distance=dis_kpc*u.kpc, frame='galactic')
    ...
    delta_0_filt = (delta_EBV==0)
    delta_EBV[delta_0_filt] = np.random.uniform(0.0, 0.02, delta_0_filt.sum())

    return EBV, delta_EBV

def cas18_extinction(bp_rp, EBV, delta_EBV):
    ...
    return AG, delta_AG, E_BP_RP, delta_E_BP_RP

def bab18_extinction(bp_rp_0, EBV, delta_EBV):
...
def else_extinction(EBV, delta_EBV):
    AG = 0.86117*3.1*EBV
    Abp = 1.06126*3.1*EBV
    Arp = 0.64753*3.1*EBV
    E_BP_RP = Abp - Arp
    ...
    return AG, delta_AG,  E_BP_RP, delta_BP_RP
