...
def get_GAIA(ds):
    ...
    En, Ltotal, Lz, Lperp = Potential.En_L_calc(
        *ds.evaluate(['x', 'y', 'z', 'vx', 'vy', 'vz']))
    ...
    circ = tc.calc_circ( En, En_circ, Lz, Lz_circ) # ds.count(),
    ...
    ds.add_virtual_columns_cartesian_to_polar(
        x='(x-8.2)', radius_out='R', azimuth_out='rPhi', radians=True)

    # Velocities in polar coordinates
    ds.add_virtual_columns_cartesian_velocities_to_polar(
        x='(x-8.2)', vx='vx_apex', vy='vy_apex', vr_out='vR', vazimuth_out='_vphi', propagate_uncertainties=False)
    ds.add_virtual_column('vphi', '-_vphi')  # flip sign for convention

    # position depending correction for the LSR
    ds.add_column('_vx', ds.vx.values+11.1 -
                  ds.variables['vlsr']*np.sin(ds.evaluate('rPhi')))
    ds.add_column('_vy', ds.vy.values+12.24 -
                  ds.variables['vlsr']*np.cos(ds.evaluate('rPhi')))
