...
def plot_original_data(df, savepath=None):
    ...
    for i in range(6):
        plt.sca(axs[int(i/3), i%3])
        im = df.viz.heatmap(x_axis[i], y_axis[i], f='log', colorbar=False, limits=limits[i], **c_args)
        #im.set_clim(0, 6)
        plt.xlabel(xlabels[i])
        plt.ylabel(ylabels[i])
...
def plot_chi2_vs_Dsquared(df, degrees_freedom=4, savepath=None):
    ...
    axs.viz.heatmap(x, chi2.pdf(x, df=k), alpha=0.8, color='red', linestyle='dashed', label=f'$\chi^2$')
    axs.legend()
