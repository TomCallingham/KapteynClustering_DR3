...
def next_members(tree_dic, Z1, Z2, i, N_cluster):
    tree_dic[i + N_cluster] = tree_dic[Z1[i]] + tree_dic[Z2[i]]
    return tree_dic
...
def find_tree(Z, i_max=None, prune=False):
    ...
    for i in range(N_cluster)[:i_max]:
        tree_dic = next_members(tree_dic, Z1, Z2, i, N_cluster1)
    print("Tree found")
    if prune:
...
def scale_features(stars, features, scales, plot=False):
    scale_data = {}
    percent = 5
    for p in features:
