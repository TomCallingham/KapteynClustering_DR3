...
def check_xykeys(xy_keys):
    ...
    if type(xy_keys) is list:
        return xy_keys, False, None
    elif type(xy_keys) is tuple:
...
def create_axis(xy_keys, shape=None, sharex=False, sharey=False, xsize_scale = 9, ysize_scale=7.5):
    if shape is None:
        N_plots = len(xy_keys)
        if N_plots>3:
...
def set_ax_label(Fig, axs, xy_keys, sharex=False, sharey=False):
...
def set_ax_lims(Fig, axs, xy_keys, Lims=lims0):
    for [xkey,ykey] in xy_keys:
        ax = axs[ykey][xkey]
        try:
...
def set_ax_pos(Fig, axs, sharex=False, sharey=False):
...
def set_G_labels(Fig,ax,Groups, G_colours, label_dic={}, below=True, leg_args={"ncol":4}):
    group_legs = {}
    for g in Groups:
    ...
    Fig.legend(handles=list(group_legs.values()), fancybox=True, shadow=True, **leg_args)
    return

def simple_scatter(stars, xy_keys, Groups, groups, G_colours, Fig, axs, group_order={},
    ...
    for [xkey,ykey] in xy_keys:
        ...
        for i_g,g in enumerate(plot_Groups):
            ...
            ax.scatter(x[g_filt], y[g_filt], c= G_colours[g], zorder=z_order, **arg_dic)

        if fluff:
            g_filt = (groups==-1)
            ax.scatter(x[g_filt], y[g_filt], c= "lightgrey", zorder=-99, **arg_dic)
