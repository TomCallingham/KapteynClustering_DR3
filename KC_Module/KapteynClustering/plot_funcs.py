...
def plot_simple_scatter(stars, xy_keys, Groups, groups,
    ...
    fig, axs = ps.create_axis(
        xy_keys, shape=shape, sharex=False, sharey=shared)
    if G_colours is None:
        G_colours = ps.get_G_colours(Groups)
    ps.simple_scatter(stars, xy_keys, Groups, groups, G_colours, axs)

    ps.set_ax_label(axs, xy_keys, sharex=False, sharey=shared)
    ps.set_ax_lims(axs, xy_keys, Lims=Lims)
