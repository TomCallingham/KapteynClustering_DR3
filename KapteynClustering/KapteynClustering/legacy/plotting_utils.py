...
def get_cmap(df_minsig):
    ...
    cmap, norm = colors.from_levels_and_colors(
        unique_labels, cmaplist, extend='max')
...
def plot_IOM_subspaces(df, minsig=3, savepath=None):
    ...
    y_axis = ['En/10e4', 'Lperp/10e2', 'Lz/10e2',
              'En/10e4', 'Lperp/10e2', 'En/10e4']
    ...
    for i in range(6):
        ...
        df.scatter(x_axis[i], y_axis[i], s=0.5,
                   c='lightgrey', alpha=0.1, length_limit=60000)
...
def plot_v_subspaces(df, minsig=3, savepath=None):
    ...
    background = df[(df.vR > -500) & (df.vR < 500) &
                    (df.vphi > -500) & (df.vphi < 500)]
    ...
    for i in range(3):
        ...
        plt.ylim([-570, 570])
        # )#, length_limit=60000)
        background.scatter(x_axis[i], y_axis[i],
                           s=0.5, c='lightgrey', alpha=0.1)
...
def plot_membership_probability(df, clusterNr, savepath=None):
    ...
    y_axis = ['En/10e4', 'Lperp/10e3', 'Lz/10e3',
              'En/10e4', 'Lperp/10e3', 'En/10e4']
...
def plot_outliers_vspace(df, clusterNr, savepath=None):
    ...
    for i in range(3):
        plt.sca(axs[0][i])
        cluster.scatter(x_axis[i], y_axis[i], s=5,
                        c=cluster.v_space_labels.values, cmap=cmap1, norm=norm, xlabel=' ')

        plt.sca(axs[1][i])
        cluster.scatter(x_axis[i], y_axis[i], s=5,
                        c=cluster.membership_probability.values, cmap=cmap2, xlabel=' ')

        plt.sca(axs[2][i])
        cluster.scatter(x_axis[i], y_axis[i], s=5, c='lightgrey', alpha=0.5)
        cluster[(cluster.v_space_labels == 0) & (cluster.membership_probability < 0.05)].scatter(
            x_axis[i], y_axis[i], s=10, c='blue')
...
def plot_clusters_over_subpanels(df, feature1, feature2, minsig=3, savepath=None):
    ...
    for idx in range(15):
        ...
        # )#, length_limit = 60000)
        df.scatter(feature1, feature2, s=0.5, c='lightgrey', alpha=0.1)
        v_slice.scatter(feature1, feature2, s=5,
                        c=v_slice.labels.values, cmap=cmap, norm=norm, alpha=0.8)
...
def plot_original_data(df, savepath=None):
    ...
    for i in range(6):
        plt.sca(axs[int(i/3), i % 3])
        im = df.plot(x_axis[i], y_axis[i], f='log',
                     colorbar=False, limits=limits[i], **c_args)
        #im.set_clim(0, 6)
        plt.xlabel(xlabels[i])
        plt.ylabel(ylabels[i])
    ...
    fig.colorbar(sm(norm=mpl.colors.Normalize(vmin=0, vmax=6),
                 cmap='afmhot'), cax=cb, label="log count (*)")
...
def plot_subgroup_vspace(df, clusterNr, zoom=False, savepath=None):
    ...
    for i in range(3):
        ...
        cluster.scatter(x_axis[i], y_axis[i], s=15,
...
def plot_cluster_vspace(df, clusterNr, savepath=None):
    ...
    # )#, length_limit=60000)
    df.scatter('Lz/10e2', 'En/10e4', s=0.5, c='lightgrey', alpha=0.1)
    cluster.scatter('Lz/10e2', 'En/10e4', s=2, c=cluster.labels.values,
                    cmap=cmap, norm=norm, alpha=1, label=f'{clusterNr}')
...
def plot_substructure_vspace(df, clusterNr, savepath=None):
    ...
    cluster_colors = [cmaplist[col]
                      for col in cluster.v_space_labels_substructure.values]
...
def plot_substructure_vspace_En_Lz(df, clusterNr, name, savepath=None):
    ...
    cluster_colors = [cmaplist[col % len(
        cmaplist)] for col in cluster.v_space_labels_substructure.values]
    ...
    norm = mpl.colors.Normalize(vmin=0, vmax=len(
        np.unique(df.labels_substructure.values)))

    plt.sca(axs[3])
    axs[3].set_xlim(-0.5, 0.5)
    axs[3].set_ylim(-1.75, -0.15)
    df.scatter('Lz/10e2', 'En/10e4', s=0.5, c='lightgrey',
               alpha=0.1)  # , length_limit=60000)
    cluster.scatter('Lz/10e2', 'En/10e4', s=2, c=cluster.labels_substructure.values,
                    cmap=cmap, norm=norm, alpha=0.8, label=f'{name}')
    plt.xlabel('$L_z$ [$10^3$ kpc km/s]')
    plt.ylabel('$E$ [$10^5$ km$^2$/s$^2$]')
...
def plot_v_space_dispersions(dispersion_xyz, savepath=None):
    ...
    axs[0].hist(dispersion_xyz[:, 0], bins=58, range=[
                0, 125], color='steelblue', alpha=0.5)
    axs[0].set_xlabel('$\sigma_1$ [km/s]')

    axs[1].hist(dispersion_xyz[:, 1], bins=58, range=[
                0, 125], color='steelblue', alpha=0.5)
    axs[1].set_xlabel('$\sigma_2$ [km/s]')

    axs[2].hist(dispersion_xyz[:, 2], bins=58, range=[
                0, 125], color='steelblue', alpha=0.5)
    axs[2].set_xlabel('$\sigma_3$ [km/s]')

    axs[3].hist(dispersion_xyz[:, 3], bins=58, range=[
                0, 125], color='steelblue', alpha=0.5)
...
def plot_chi2_vs_Dsquared(df, degrees_freedom=4, savepath=None):
    ...
    axs.plot(x, chi2.pdf(x, df=k), alpha=0.8, color='red',
             linestyle='dashed', label=f'$\chi^2$')
    axs.legend()
...
def plot_significance_levels(df, significance_levels=[3, 4, 5, 6, 8, 10], savepath=None):
    ...
    for idx, n_sigma in enumerate(significance_levels):
        ...
        # , length_limit = 60000, selection = df.En<-50000)
        df.scatter('Lz', 'En', s=0.5, c='lightgrey', alpha=0.1)
        df_slice = df[df[colname] > 0]
        df_slice.scatter('Lz', 'En', s=0.5,
                         c=df_slice[colname].values, cmap=cmap, alpha=0.6)
...
def plot_clusters_by_significance(df, minsig=3, savepath=None):
    ...
    df[(df.Lz < 4000) & (df.Lz > -4000)].scatter('Lz/10e2', 'En/10e4',
                                                 s=0.5, c='lightgrey', alpha=0.1)  # , length_limit=60000)
